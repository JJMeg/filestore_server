...
func InitiateMultipartUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.Write(util.MewRespMsg(-1, "params invalid", nil).JSONBytes())
		return
	}
	...
	// 5. 初始化信息返回给客户端
	w.Write(util.MewRespMsg(0, "OK", upInfo).JSONBytes())
	return
}
...
func UploadPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.Write(util.MewRespMsg(-1, "upload part fail", nil).JSONBytes())
		return
	}
	defer fd.Close()
	...
	//  5. 返回结果给客户端
	w.Write(util.MewRespMsg(0, "OK", nil).JSONBytes())
	return
}

// CompleteUploadHander：通知上传合并接口
func CompleteUploadHander(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.Write(util.MewRespMsg(-1, "complete upload fail", nil).JSONBytes())
		return
	}
	totalCount := 0
	chunkCount := 0
	...
	if totalCount != chunkCount {
		w.Write(util.MewRespMsg(-2, "invalid request", nil).JSONBytes())
		return
	}
